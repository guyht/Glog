...
Glog.prototype.load_plugins = function(options, cb) {
    ...
    var npmops = {};
    npmops.cwd = 'plugins/';
    npmops.env = process.env;
    var npm = spawn('npm', ['install'], npmops);
    npm.stdout.on('data', function(data){
      console.log('stdout: ' + data);
    });
    npm.on('exit', function(code){
      console.log('stdout: npm exited with code: ' + code);
      for(i=0;i<options.plugins.length;i++) {
          try {
              ...
              try {
                var p2 = require('../plugins/node_modules/' + options.plugins[i]);
                p2.load(GlogPlugin);
              } catch(e2) {
