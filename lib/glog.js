...
Glog.prototype.load_configs = function(cb) {
    ...
    if(!config.port) {
        fn.handle_error('port is not defined in config/default.json.  This should be set to the port number you wish the blog to run on');
        process.exit();
    }

    if(!config.blog_repository) {
        fn.handle_error('blog_repository is not defined in config/default.json.  This should point to the git repository of the blog content');
        process.exit();
    }

    if(!config.author) {
        fn.handle_error('author is not defined in config/default.json.  This should be the default blog author');
        process.exit();
    }
